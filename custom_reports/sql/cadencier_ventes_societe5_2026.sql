...
--
-- Paramètres :
--   2026        → année entière (ex: 2026)
--   5  → ID société (ex: 1)
--   NULL → tableau d'IDs catégorie (ex: ARRAY[1,2,3])
...
eligible AS (
    ...
    WHERE pt.type   = 'consu'
      ...
      AND (NULL IS NULL OR pt.categ_id = ANY(NULL::int[]))
),

-- ── 3. Ventes Sale agrégées par produit et par mois ────────
-- Date : >= '2026-01-01' AND <= '2026-12-31'
...
sale_agg AS (
    ...
    WHERE so.state      IN ('sale', 'done')
      AND so.company_id  = 5
      AND so.date_order >= (make_date(2026, 1, 1))::timestamp
      AND so.date_order <  (make_date(2026, 12, 31))::timestamp  -- <= 31 déc minuit = comportement ORM
...
pos_agg AS (
    ...
    WHERE po.state      IN ('done', 'paid', 'invoiced')
      AND po.company_id  = 5
      AND po.date_order >= (make_date(2026, 1, 1))::timestamp
      AND po.date_order <  (make_date(2026, 12, 31))::timestamp
...
LEFT JOIN product_company_status pcs ON pcs.product_id = pt.id AND pcs.company_id = 5
LEFT JOIN product_status         ps  ON ps.id           = pcs.status_id
